                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            ...
                            <button
                                  ...
                                  data-release-title="{{ $stipend->stipendRelease->title ?? 'N/A' }}"
                                  data-amount="{{ number_format((float)$stipend->amount_received, 2) }}"
                                  data-status="{{ strtoupper(str_replace('_',' ', $stipend->status)) }}"
                              >
                                  Delete
                              </button>
                        </td>
    ...
    <div class="modal-content">
      ...
      <form method="POST" action="{{ route('coordinator.stipends.bulk-assign-v2') }}" id="bulkScheduleForm">
        ...
        <div class="sticky-actions d-flex justify-content-end gap-2">
          ...
          <button class="btn btn-bisu btn-sm" type="submit" id="submitBtn">
            <span id="btnText">Submit & Notify Scholars</span>
        </button>
        </div>
      </form>
...
document.addEventListener('click', function(e){
  ...
  const base = @json(route('coordinator.stipends.destroy', ['id' => '___ID___']));
