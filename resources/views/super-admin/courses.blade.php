...
<div class="p-6">  {{-- Padding for content --}}
    ...
    <div class="flex justify-end mb-6">
        <a href="{{ route('admin.courses.create') }}" class="inline-flex items-center bg-black text-black hover:bg-gray-800 font-bold py-3 px-6 rounded-lg shadow-md transition duration-200">
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="overflow-x-auto max-h-[calc(100vh-250px)] overflow-y-auto">  {{-- Strict height for internal scrolling --}}
            <table class="table-auto w-full border-collapse text-center min-w-full" id="coursesTable">  {{-- Added ID for JS filtering --}}
                ...
                <tbody>
                    @foreach($courses ?? [] as $course)
                        <tr class="hover:bg-gray-50 transition duration-150 even:bg-gray-25">
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $course->course_name }}</td>
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $course->course_description ?? 'N/A' }}</td>
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $course->college->college_name ?? 'N/A' }}</td>
                            <td class="border border-gray-300 px-3 py-2 space-x-2">
                                <a href="{{ route('admin.courses.edit', $course->id) }}" class="inline-flex items-center bg-blue-500 hover:bg-blue-600 text-black font-medium py-1 px-3 rounded shadow transition duration-200 text-sm">
                                    <span class="mr-1">✏️</span> Edit
                                </a>
                                <a href="{{ route('admin.courses.delete', $course->id) }}" class="inline-flex items-center bg-red-500 hover:bg-red-600 text-black font-medium py-1 px-3 rounded shadow transition duration-200 text-sm">
                    ...
                    @if(empty($courses))
                        <tr id="noResultsRow">
                            <td colspan="4" class="px-3 py-4 text-gray-500 text-center">No courses found. <a href="{{ route('admin.courses.create') }}" class="text-blue-500 underline hover:text-blue-700">Add one now</a>.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
