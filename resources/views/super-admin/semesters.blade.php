...
<div class="p-6">  {{-- Padding for content --}}
    ...
    <div class="flex justify-end mb-6">
        <a href="{{ route('admin.semesters.create') }}" class="inline-flex items-center bg-black text-black hover:bg-gray-800 font-bold py-3 px-6 rounded-lg shadow-md transition duration-200">
                ...
                <tbody>
                    @foreach($semesters ?? [] as $semester)
                        <tr class="hover:bg-gray-50 transition duration-150 even:bg-gray-25">
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $semester->term }}</td>
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $semester->academic_year }}</td>
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $semester->start_date }}</td>
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $semester->end_date }}</td>
                            <td class="border border-gray-300 px-3 py-2 text-gray-800">{{ $semester->is_current ? 'Yes' : 'No' }}</td>
                            <td class="border border-gray-300 px-3 py-2 space-x-2">
                                <a href="{{ route('admin.semesters.edit', $semester->id) }}" class="inline-flex items-center bg-blue-500 hover:bg-blue-600 text-black font-medium py-1 px-3 rounded shadow transition duration-200 text-sm">
                                    <span class="mr-1">✏️</span> Edit
                                </a>
                                <a href="{{ route('admin.semesters.delete', $semester->id) }}" class="inline-flex items-center bg-red-500 hover:bg-red-600 text-black font-medium py-1 px-3 rounded shadow transition duration-200 text-sm">
                    ...
                    @if(empty($semesters))
                        <tr>
                            <td colspan="6" class="px-3 py-4 text-gray-500 text-center">No semesters found. <a href="{{ route('admin.semesters.create') }}" class="text-blue-500 underline hover:text-blue-700">Add one now</a>.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
