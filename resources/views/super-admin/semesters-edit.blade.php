...
<form method="POST" action="{{ route('admin.semesters.update', $semester->id) }}">
    ...
    <button type="submit" class="bg-yellow-500 text-white px-4 py-2 rounded">Update Semester</button>
...
<form method="POST" action="{{ route('admin.semesters.destroy', $semester->id) }}" class="mt-4">
    @csrf @method('DELETE')
    <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded" onclick="return confirm('Delete?')">Delete Semester</button>
</form>
@endsection