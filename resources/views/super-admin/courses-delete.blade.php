{{-- resources/views/super-admin/courses-delete.blade.php --}}
@extends('layouts.app')
...
<div class="min-h-screen flex items-center justify-center bg-gray-100">  {{-- Centers the content like a small modal --}}
    <div class="max-w-sm mx-auto bg-white shadow-lg rounded-lg p-6">  {{-- Smaller width for compactness --}}
        ...
        <div class="bg-gray-50 p-4 rounded-lg mb-4 border">
            <h2 class="font-semibold text-gray-800">{{ $course->course_name }}</h2>
            <p class="text-sm text-gray-600 mt-2">{{ $course->course_description ?? 'No description' }}</p>
            <p class="text-sm text-gray-600">College: {{ $course->college->college_name ?? 'N/A' }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.courses.destroy', $course->id) }}">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('admin.dashboard', ['page' => 'courses']) }}" class="inline-flex items-center text-gray-600 hover:text-gray-800 font-medium py-2 px-4 transition duration-200 text-sm">
