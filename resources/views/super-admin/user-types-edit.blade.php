...
<form method="POST" action="{{ route('admin.user-types.update', $userType->id) }}">
    @csrf @method('PUT')
    <input type="text" name="name" value="{{ $userType->name }}" class="border p-2 w-full mb-4" required>
    <textarea name="description" class="border p-2 w-full mb-4" rows="3">{{ $userType->description }}</textarea>  {{-- Fixed: Removed placeholder, added content --}}
